...
public class Node {
    ...
    public Node(String name) {
        ...
        routingTable.put(name, new RouteEntry(name, name, 0, RouteEntry.State.ACTIVE));
    }
    ...
    public void addNeighbor(String neighborName, int cost) {
        ...
        routingTable.put(neighborName, new RouteEntry(neighborName, neighborName, cost, RouteEntry.State.ACTIVE));
    }
        ...
        for (Map.Entry<String, Integer> entry : neighborVector.entrySet()) {
            ...
            if (shouldUpdate) {
                ...
                if (isActuallyDifferent) {
                    RouteEntry.State newState = (newCost >= INF) ? RouteEntry.State.INACTIVE : RouteEntry.State.ACTIVE;
                    routingTable.put(destination, new RouteEntry(destination, neighborName, newCost, newState));
                    changed = true;
                }
            }
        }
    ...
    public void removeNeighbor(String neighborName) {
        ...
        for (Map.Entry<String, RouteEntry> entry : routingTable.entrySet()) {
            ...
            if (route.getNextHop().equals(neighborName)) {
                route.setCost(INF);
                route.setState(RouteEntry.State.INACTIVE);
            }
        }
